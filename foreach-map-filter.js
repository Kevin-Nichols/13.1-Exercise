...
function vowelCount(str){
   ...
   let newObj = {};
    newArr.forEach(function(letter){
        let convertLower = letter.toLowerCase();
        if('aeiou'.indexOf(convertLower) !== -1){
            if(newObj[convertLower]){
                newObj[convertLower]++;
            } else {
                newObj[convertLower] = 1;
            }
        }
    });
    return newObj;
...
function valTimesIndex(arr){
    let newArr = [];
    arr.map(function(value, indx){
        newArr.push(value * indx);
    });
    return newArr;
}
